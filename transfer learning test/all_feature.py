...
def _stack_features(all_dict, features):
    if len(all_dict) < 1:
        all_dict = {k:[] for k in features.keys()}
    for split_id in features.keys():
        all_dict[split_id].append(features[split_id])
    return all_dict
...
def _one_cluster_model(model_name: str, train_num: int, input_dt, label_dt, source_type: bool, save_preds=False):
    """
    ...
    for coord_pm in [True, False]:
        ...
        all_source_features = _stack_features(all_source_features, source_encode)
        all_train_target_features = _stack_features(all_train_target_features, train_target_encode)
        all_valid_features = _stack_features(all_valid_features, valid_encode)
    all_source_features = {k:np.hstack(all_source_features[k]) for k in all_source_features.keys()}
    all_train_target_features = {k:np.hstack(all_train_target_features[k]) for k in all_train_target_features.keys()}
    all_valid_features = {k:np.hstack(all_valid_features[k]) for k in all_valid_features.keys()}

    # Define input data for transfer learning
    source_input, train_target_input, valid_input = _combine_encode_input(train_test_data_id, source_type, source_encode, train_target_encode, valid_encode)
