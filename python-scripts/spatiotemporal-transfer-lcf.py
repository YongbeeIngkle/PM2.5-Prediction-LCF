...
def _combine_encode_input(train_test_data_id, east_source: bool, source_encode: dict, train_target_encode: dict, valid_encode: dict):
...
def _one_cluster_model(model_name, train_num, input_dt, label_dt, coord_pm, east_source, save_preds=False):
    ...
    if coord_pm:
        model_save_name = f"LCF_coord-{model_name}"
        encoder_save_dir = "trained models/encode feature/"
    else:
        model_save_name = f"LCF_whole-{model_name}"
    ...
    if east_source:
        ...
        encoder_save_dir = f"{encoder_save_dir}whole/"
    ...
    source_input, train_target_input, valid_input = _combine_encode_input(train_test_data_id, east_source, source_encode, train_target_encode, valid_encode)
...
if __name__=='__main__':
    all_models = ["NnwGbr", "RulsifGbr", "Kmm", "Kliep"]
