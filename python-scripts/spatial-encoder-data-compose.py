...
class StationAllocate:
    def __init__(self, source_data, train_target_data, valid_data, source_label, train_target_label, station_num):
    ...
    def _compute_date_set(self, date):
        ...
        date_source_label = self.source_label.loc[self.source_data["day"]==date]
        date_train_target_label = self.train_target_label.loc[self.train_target_data["day"]==date]
        ...
        return date_source_dataset, date_train_target_dataset, date_valid_dataset
    ...
    def _allocate_all_data(self):
        ...
        for date_num, date in enumerate(all_dates):
            ...
            date_source, date_train_target, date_valid = self._compute_date_set(date)
...
class DataCompose:
    ...
    def _split_train_valid_cmaq(self):
        ...
        for split_id in self.train_valid_data_id.keys():
            ...
            source_index, train_target_index, valid_index = cluster_train_valid_index(set_index)
    ...
    def _allocate_near_data(self):
        ...
        for split_id in self.train_valid_data_id.keys():
            ...
            if self.save_merged_data:
                station_allocate = StationAllocate(source_input, train_target_input, valid_input, source_label, train_target_label, 12)
                source_data, train_target_data, valid_data = station_allocate.composed_inputs
                np.savez(f"{self.data_path}split-{split_id}/nearest_dataset.npz", source=source_data, train_target=train_target_data, valid=valid_data)
            else:
                save_npz = np.load(f"{self.data_path}split-{split_id}/nearest_dataset.npz")
                source_data, train_target_data, valid_data = save_npz["source"], save_npz["train_target"], save_npz["valid"]
...
def _one_cluster_model(train_num, input_dt, label_dt):
    ...
    DataCompose(input_dt, label_dt, train_test_data_id, False, True, compose_path)
...
if __name__=='__main__':
    ...
    for train_num in train_numbers:
        _one_cluster_model(train_num, input_dt, label_dt)
