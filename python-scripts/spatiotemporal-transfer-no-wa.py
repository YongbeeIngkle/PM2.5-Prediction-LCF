...
def run_tradaboost_model(source_x, source_y, target_x, target_y, test_x, test_y):
    ...
    model_tradaboost = TwoStageTrAdaBoostR2(DecisionTreeRegressor(**params), n_estimators = 2, Xt = target_x, yt = target_y, random_state = 2) ##DecisionTreeRegressor(max_depth = 6), n_estimators = 100, Xt = target_x, yt = target_y, random_state = 0)
    model_tradaboost.fit(source_x, source_y, target_x, target_y)

    pred_tradaboost = model_tradaboost.predict(test_x)
    pred_tradaboost = pred_tradaboost.ravel()

    # ans = True if True in np.isnan(np.array(pred_tradaboost)) else False
    # print(pred_tradaboost.ravel())
...
############## Kriging Regression ##############
def run_kriging_model(train_pred_x, train_coords_x, train_y, test_pred_x, test_coords_x, test_y):
    ...
    model_kriging.fit(train_pred_x, train_coords_x, train_y)

    print("Regression Score: ", model_kriging.regression_model.score(test_pred_x, test_y))
    print("RK score: ", model_kriging.score(test_pred_x, test_coords_x, test_y))
...
def main():
    ...
    source_data[cols_to_scale] = ss.fit_transform(source_data[cols_to_scale])
    ...
    for sp in mn_range_list:
        ...
        # tradaboost_file = open(f"results/results-minmax-no-wa/tradaboost-results", "a+") ## results/
        # adaboost_file = open(f"results/results-minmax-no-wa/adaboost-results", "a+")
        # kmm_file = open(f"results/results-minmax-no-wa/kmm-results", "a+")
        # nnw_file = open(f"results/results-minmax-no-wa/nnw-results", "a+")
        # ulsif_file = open(f"results/results-minmax-no-wa/ulsif-results", "a+")
        # rulsif_file = open(f"results/results-minmax-no-wa/rulsif-results", "a+")
        # rf_file = open(f"results/results-minmax-no-wa/rf-results", "a+")
        # gbr_file = open(f"results/results-minmax-no-wa/gbr-results", "a+")
        kgbr_file = open(f"results/results-minmax-no-wa/kgbr-results", "a+")
        kliep_gbr_file = open(f"results/results-minmax-no-wa/kliep-gbr-results", "a+")
        rulsif_gbr_file = open(f"results/results-minmax-no-wa/rulsif-gbr-results", "a+")
        nnw_gbr_file = open(f"results/results-minmax-no-wa/nnw-gbr-results", "a+")
        ...
        for idx in range(0,10):
            ...
            target_data = target_data.drop(columns=['cmaq_id'])
            target_data = target_data.reset_index(drop = True)

            target_data[cols_to_scale] = ss.fit_transform(target_data[cols_to_scale])
            target_x = target_data.drop(columns=["pm25_value"]).to_numpy()
            ...
            train_df = train_df.drop(columns=['cmaq_id'])
            ...
            test_df = test_df.drop(columns=['cmaq_id'])
            ...
            # #### create geo training data, train = target + source
            # geotrain_cmaq = target_cmaq + source_cmaq
            # geotrain_df = geo_monitoring_df[geo_monitoring_df['cmaq_id'].isin(geotrain_cmaq)]
            # geotrain_df = geotrain_df.drop(columns=['cmaq_id'])
            # geotrain_df = geotrain_df.reset_index(drop = True)
            #
            # geotrain_coords_df = geotrain_df[['lat', 'lon']]
            # geotrain_label = geotrain_df["pm25_value"]
            # geotrain_pred_df = geotrain_df.drop(columns=['lat', 'lon', 'pm25_value'])
            #
            # geotrain_pred_df[cols_to_scale] = ss.fit_transform(geotrain_pred_df[cols_to_scale])
            # train_pred_x = geotrain_pred_df.to_numpy()
            # train_pred_x = np.nan_to_num(train_pred_x)
            # train_coords_x = geotrain_coords_df.to_numpy()
            # train_y = geotrain_label.to_numpy()
            # train_y = np.nan_to_num(train_y)
            #
            # #### read the geotest cmaqs
            # geotest_cmaq = cmaq_ids["test"]
            # geotest_df = geo_monitoring_df[geo_monitoring_df['cmaq_id'].isin(geotest_cmaq)]
            # geotest_df = geotest_df.drop(columns=['cmaq_id'])
            # geotest_df = geotest_df.reset_index(drop = True)
            #
            # geotest_coords_df = geotest_df[['lat', 'lon']]
            # geotest_label = geotest_df["pm25_value"]
            # geotest_pred_df = geotest_df.drop(columns=['lat', 'lon', 'pm25_value'])
            #
            # geotest_pred_df[cols_to_scale] = ss.fit_transform(geotest_pred_df[cols_to_scale])
            # test_pred_x = geotest_pred_df.to_numpy()
            # test_pred_x = np.nan_to_num(test_pred_x)
            # test_coords_x = geotest_coords_df.to_numpy()
            # test_y = geotest_label.to_numpy()
            # test_y = np.nan_to_num(test_y)
            ...
            # r2_corr_kriging, r2_kriging, rmse_kriging, mae_kriging =
            # run_kriging_model(train_pred_x, train_coords_x, train_y, test_pred_x, test_coords_x, test_y)


            # print(r2_corr_tradaboost, r2_tradaboost, rmse_tradaboost, mae_tradaboost)
            ...
            # print(r2_corr_kgbr, r2_kgbr, rmse_kgbr, mae_kgbr)
            # print(r2_corr_kliep_gbr, r2_kliep_gbr, rmse_kliep_gbr, mae_kliep_gbr)
            # print(r2_corr_kliep_gbr, r2_kliep_gbr, rmse_kliep_gbr, mae_kliep_gbr)
            # print(r2_corr_rulsif_gbr, r2_rulsif_gbr, rmse_rulsif_gbr, mae_rulsif_gbr)
            # print(r2_corr_nnw_gbr, r2_nnw_gbr, rmse_nnw_gbr, mae_nnw_gbr)
            # print(r2_corr_ldm_gbr, r2_ldm_gbr, rmse_ldm_gbr, mae_ldm_gbr)
            # print(r2_corr_nnw_gbr, r2_nnw_gbr, rmse_nnw_gbr, mae_nnw_gbr)
